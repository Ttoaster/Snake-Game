const canvas = document.getElementById("canvas")
const canvasContext = canvas.getContext('2d');

//import './snake.css';

class Snake {
    constructor(x, y, size) {
        this.x = x
        this.y = y
        this.size = size
        this.tail = [{ x: this.x, y: this.y }]
        this.rotateX = 0
        this.rotateY = 1
    }

    move() {
        var newRectangle;
        if (this.rotateX == 1) {
             newRectangle = {
                x: this.tail[this.tail.length - 1].x + this.size,
                y: this.tail[this.tail.length - 1].y
            }
        } else if  (this.rotateX == -1) {
             newRectangle = {
                x: this.tail[this.tail.length - 1].x - this.size,
                y: this.tail[this.tail.length - 1].y
            }
        } else if (this.rotateY == 1) {
             newRectangle = {
                x: this.tail[this.tail.length - 1].x,
                y: this.tail[this.tail.length - 1].y + this.size
            }
        } else if (this.rotateY == -1) {
             newRectangle = {
                x: this.tail[this.tail.length - 1].x,
                y: this.tail[this.tail.length - 1].y - this.size
            }
        } 
        this.tail.shift()
        this.tail.push(newRectangle)
    }
}

class Apple {
    constructor() {
        console.log("apple")
        console.log(snake.size)
        var isTouching;
        while (true) {
            isTouching = false;
            this.x = Math.floor(Math.random() * canvas.width / snake.size) * snake.size
            this.y = Math.floor(Math.random() * canvas.height / snake.size) * snake.size
            for (var i = 0; i < snake.tail.length; i++) {
                if (this.x == snake.tail[i].x && this.y == snake.tail[i].y) {
                    isTouching = true
                }
            }
            console.log(this.x , this.y)
            this.color = "red"
            this.size = snake.size
            if (!isTouching) {
                break;
            }
        }
    }
}

var snake = new Snake(20,20,20);

var apple = new Apple();

window.onload = () => {
    gameLoop();
}

function gameLoop() {
    setInterval(show, 1000/5) //here 5 is our fps value
}

function show() {
    update();
    draw();
}

function update() {
    canvasContext.clearRectangle(0, 0, canvas.width, canvas.height)
    console.log("update")
    snake.move()
    eatApple()
    checkHitWall();
}

function checkHitWall() {
    var headTail = snake.tail[snake.tail.length - 1]
    if (headTail.x == - snake.size) {
        headTail.x = canvas.width - snake.size
    } else if (headTail.x ==  canvas.width) {
        headTail.x = 0
    } else if (headTail.y == - snake.size) {
        headTail.y = canvas.height - snake.size
    } else if (headTail.y == canvas.height) {
        headTail.y = 0
    }
}
 
function eatApple() {
    if (snake.tail[snake.tail.length - 1].x == apple.x &&
        snake.tail[snake.tail.length - 1].y == apple.y) {
            snake.tail[snake.tail.length] = { x: apple.x, y: apple.y }
            apple = new Apple();
        }
}

function draw() {
    createRectangle(0,0,canvas.width, canvas.height, "black")
    createRectangle(0, 0, canvas.width, canvas.height)
    for (var i = 0; i < snake.tail.length; i++) {
        createRectangle(snake.tail[i].x + 2.5,
            snake.tail[i].y + 2.5,
            snake.size - 5,
            snake.size - 5,
            "green")
    }
    canvasContext.font = "20px Arial"
    canvasContext.fillStyle = "#00FF42"
    canvasContext.fillText("Score: " +
        (snake.tail.length -1),
        canvas.width - 120,
        20);
    createRectangle(apple.x, apple.y, apple.size, apple.size, apple.color)

}

function createRectangle(x, y, width, height, color) {
    //console.log("color: ", color);
    canvasContext.fillStyle = color
    canvasContext.fillRectangle(x,y,width,height)
}

window.addEventListener("keydown", (event) => {
    setTimeout(() => {
        if (event.code === "ArrowLeft" && snake.rotateX != 1) {
            console.log("37 leftArrow")
            snake.rotateX = -1
            snake.rotateY = 0;
        } else if (event.code === "ArrowUp" && snake.rotateY != 1) {
            console.log("upArrow")
            snake.rotateX = 0
            snake.rotateY = -1;
        } else if (event.code === "ArrowRight" && snake.rotateX != -1) {
            console.log("rightArrow")
            snake.rotateX = 1
            snake.rotateY = 0;
        }
        else if (event.code === "ArrowDown" && snake.rotateY != -1) {
            snake.rotateX = 0
            snake.rotateY = 1;
        }
    }, 1)
})
